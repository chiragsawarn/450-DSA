...
class LinkedList{
    ...
    public:
        ...
        int findStartOfLoop(){
            ...
            if(!meetingPointer) return -1;

            node * fast = head;

            while(fast != meetingPointer){
                fast = fast->next;
                meetingPointer = meetingPointer->next;
            }
            
            return meetingPointer->data;
        }
};
...
int main(){
    ...
    int startOfLoop = l1.findStartOfLoop();
    (startOfLoop == -1) ? cout<<"Loop doesn't exist!" : cout<<"Loop starts at "<<startOfLoop;
